...
func loadAndAuthenticateUser(
	userId UserId,
	authToken string,
) (User, ErrorWithStatus) {
	var user User
...
func loadGameAndAuthenticatedCreator(
	f Fundamentals,
	creatorId UserId,
	authToken string,
	gameCode GameCode,
	...
	if creator, err = loadAndAuthenticateUser(creatorId, authToken); err != nil {
		return nil, nil, err
	}

	if game, err = f.s.LoadGame(gameCode); err != nil {
		return nil, nil, err
	} else if game.Creator != creator.Id {
